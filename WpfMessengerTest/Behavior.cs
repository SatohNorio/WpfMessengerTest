...
namespace WpfMessengerTest
{
	...
	public class EnterKeyUpBehavior : Behavior<FrameworkElement>
	{
		...
		/// <summary>
		/// WpfMessengerTest.EnterKeyUpBehavior クラスの新しいインスタンスを作成します。
		/// </summary>
	...
	/// コンボボックス、テキストボックスに対応しています。入力された文字列をコマンドパラメータとして送信します。
	/// </remarks>
	public class MessengerBehavior : Behavior<FrameworkElement>
	{
		// ------------------------------------------------------------------------------------------------------------
		#region コンストラクタ
		...
		// ------------------------------------------------------------------------------------------------------------
		#region Actions プロパティ（依存関係プロパティ）

		public static MessageActionCollection GetActions(Window target)
		{
			return (MessageActionCollection)target.GetValue(ActionsProperty);
		}

		public static void SetActions(Window target, MessageActionCollection value)
		{
			target.SetValue(ActionsProperty, value);
		}

		/// <summary>
		/// メッセージアクションのコレクション を管理する依存関係プロパティ
		/// </summary>
		public static readonly DependencyProperty ActionsProperty =
			DependencyProperty.RegisterAttached("Actions", typeof(MessageActionCollection), typeof(MessengerBehavior), new UIPropertyMetadata(null, OnActionsPropertyChanged));

		private static void OnActionsPropertyChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e)
		{
			var w = sender as Window;
			var c = e.NewValue as MessageActionCollection;
			if (w != null && c != null)
			{
				c.RegisterAll(w);
			}
